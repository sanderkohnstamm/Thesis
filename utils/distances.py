...
def norm(X, Y, device=None, norm=True):
    M = (X - Y)
    if norm:
        M /= float(torch.sum(M))
    return sum(torch.linalg.norm(M, 2, 1))

def optimal_transport(X, Y):
    M = ot.dist(X, Y)
    return M

def optimal_transport_loss(X, Y, device='cpu', norm=True):
    a = torch.tensor(ot.unif(X.shape[0]), requires_grad=False).to(device)
    b = torch.tensor(ot.unif(X.shape[0]), requires_grad=False).to(device)
    M = ot.dist(X, Y)
    if norm: 
        M /= float(torch.median(M))
    loss = ot.emd2(a, b, M)
    return loss
...
def hsic_normalized(x, y, sigma=None, use_cuda=True, to_numpy=True, device='cpu'):
    """
    """
    m = int(x.size()[0])
